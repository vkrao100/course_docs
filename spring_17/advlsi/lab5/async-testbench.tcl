...
###
### This will iniitalize the simultion, change "pipeline" to your top level cell name
### vsim -novopt -sdftyp testbench=testbench.dcopt.sdf work.testbench
...
if { 1 } {
    ...
    # Reset time
    set reset_time 30ns
    set break_time 200ns
    set break_time2 200ns
    set zero_buf_cycle_break  5
    ...
    destroy .wave
    view wave
    add wave lr la rr ra -divider "control signals"
    add wave go_l go_r
}
...
while { 1 } {
    ...
    if { $last_lr_val == "1'h0" && [exa lr] == "1'h1" } {
	if { $last_lr_up_time > 0 
	     && [ltTime $cycle_time [subTime $Now $last_lr_up_time]] } {
	    set cycle_time [subTime $Now $last_lr_up_time]
    ...
    if { $last_la_val == "1'h1" && [exa la] == "1'h0" } {
    ...
    ### Break condition
    if { $cycle_time != 0 } {
	if { [gtTime $Now [addTime $last_lr_up_time [mulTime $cycle_time 4]]] } {
	    echo "Breaking loop - three times max cycle without another req"
	    #echo "$Now > [addTime $last_lr_up_time [mulTime $cycle_time 4]]"
	    break
	}
    }
...
echo "Max Cycle Time:           $cycle_time"
echo 
#echo "Run Time:                 $Now"
...
while { 1 } {
    ...
    if { $ra_up == 0 && [exa ra] == "1'h1" } {
	set ra_up $Now;
    }
    # set bl_go when la goes low.
    if { $bl_go == 0 && [exa la] == "1'h0" } {
	set bl_go 1
    }
    # set la_up to time of first rising rr
    if { $la_up == 0 && [exa la] == "1'h1" && $bl_go == 1} {
	set la_up $Now;
	...
	if { $all_tokens >= $tokens &&
	    ...
	    ##set cycle_time [addTime $lhs_delay $cycle_time]
	    echo "cycle time set to $cycle_time"
	    echo "now - last la up: $Now $last_lr_up_time = $cycle_time"
