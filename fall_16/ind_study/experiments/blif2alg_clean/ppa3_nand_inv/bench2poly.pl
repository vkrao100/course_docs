...
# Modified by Xiaojun Sun @ 03/22/2016
# Add: minor bug fix, 2-input XOR compatible

# Created by Xiaojun Sun @ 04/29/2015
# Objective: convert .bench benchmarks to polynomials for generic FSM traversal
...
open POLY, ">", $ARGV[0].".sing";  # change the name
open TOPO, ">", $ARGV[0].".graph";
...
while(<>){
	...
	if($read_in =~ /XOR\(/){ # newly added in case of XOR benchmark manually generated
	  $tmp =abstract($items[2]);
	  &write_xor($items[0],$tmp);
	  next;
	}
...
#	  print RED "$items[0]+1+$tmp,\n";
	...
	if($read_in =~ /BUF\(/){ #BUF gate, directly write; added by UG
	  $tmp = &abstract($items[2]);
	  
	  print POLY "$items[0]+$tmp,\n";

	  
	 #  if(!looks_like_number($items[0]))
	 #  {
	 #  	$idx = &query($items[0]);
	 # 		print TOPO "$idx ";	
	 #  }
	 ...
	 #  	print TOPO "$idx\n";
	 #  }
	  if(!looks_like_number($items[0]) and !looks_like_number($tmp))
	  {
	  	$idx = &query($items[0]);
		print TOPO "$idx ";	
		$idx = &query($tmp);
...
sub write_xor{  #XOR is by default 2-input
  my $left = $_[0];
  my $right = $_[1];
  my @term = split /,/,$right;
  if(!defined($term[2]))  # which means this is 2-input AND
